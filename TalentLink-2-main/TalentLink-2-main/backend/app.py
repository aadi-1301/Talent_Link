...
from models import db, User, Profile, Project, Proposal, Contract, Message, Review, Notification, ProjectMilestone, MilestoneUpdate, Payment, TimeEntry, SkillEndorsement
import json
import logging
...
# Configure JWT to not require auth for OPTIONS requests
...
def send_message():
    try:
        ...
        # Create notification for receiver (not for temporary messages)
        if not is_temporary:
            notif = Notification(
                user_id=receiver_id,
                type='new_message',
...
def get_messages():
    ...
    # Filter temporary messages if needed
    if not include_temporary:
        query = query.filter(Message.is_temporary == False)
    ...
    # Delete temporary messages that have been read
    temp_messages_to_delete = Message.query.filter(
        Message.sender_id == other_user_id,
        Message.receiver_id == current_user_id,
        Message.is_temporary == True,
        Message.is_read == True
    ).all()
    
    for msg in temp_messages_to_delete:
        db.session.delete(msg)
    
    if temp_messages_to_delete:
        db.session.commit()
...
def create_review():
    ...
    try:
        ...
        # Validate required fields
        if not data.get('project_id') or not data.get('reviewee_id') or not data.get('rating'):
        ...
        if current_user_id != project.client_id and current_user_id != reviewee_id:
            # Check if user is the freelancer on a contract
            contract = Contract.query.filter_by(project_id=project.id).first()
            if not contract or contract.freelancer_id != current_user_id:
                return jsonify({'error': 'You are not authorized to review this project'}), 403
        ...
        if existing_review:
            return jsonify({'error': 'You have already reviewed this user for this project'}), 400
        ...
        notif = Notification(
            user_id=reviewee_id,
            type='new_review',
            content=f'{reviewer.name} left you a {rating}-star review'
        )
        db.session.add(notif)
        db.session.commit()
...
@app.route('/api/contracts/<int:contract_id>/can-review', methods=['GET'])
@jwt_required()
def can_review_contract(contract_id):
    ...
    try:
        ...
        elif contract.freelancer_id == current_user_id:
            reviewee_id = contract.project.client_id
        else:
            return jsonify({'can_review': False, 'reason': 'Not authorized'}), 403
        ...
        return jsonify({
            'can_review': True,
            'reviewee_id': reviewee_id,
            'reviewee_name': User.query.get(reviewee_id).name
        })
        
    except Exception as e:
        app.logger.error(f"Error checking review status: {str(e)}")
        return jsonify({'error': 'Failed to check review status'}), 500
...
@app.route('/api/contracts/<int:contract_id>/time-entries', methods=['POST'])
@jwt_required()
def create_time_entry(contract_id):
    """Log time spent on a contract"""
    current_user_id = int(get_jwt_identity())
    contract = Contract.query.get_or_404(contract_id)
    
    if contract.freelancer_id != current_user_id:
        return jsonify({'error': 'Only the assigned freelancer can log time'}), 403
    ...
    # Notify client
    notif = Notification(
        user_id=contract.project.client_id,
        type='time_logged',
        content=f'{contract.freelancer.name} logged {entry.hours} hours on "{contract.project.title}"'
    )
    db.session.add(notif)
    db.session.commit()
...
@app.route('/api/contracts/<int:contract_id>/time-entries', methods=['GET'])
@jwt_required()
def get_time_entries(contract_id):
    ...
    entries = TimeEntry.query.filter_by(contract_id=contract_id).order_by(TimeEntry.date.desc()).all()
...
def delete_time_entry(entry_id):
    ...
    if entry.freelancer_id != current_user_id:
        return jsonify({'error': 'Unauthorized'}), 403
...
@app.route('/api/users/<int:user_id>/endorse', methods=['POST'])
@jwt_required()
def endorse_skill(user_id):
    ...
    user = User.query.get_or_404(user_id)
    if user.role != 'freelancer':
        return jsonify({'error': 'Can only endorse freelancers'}), 400
    ...
    notif = Notification(
        user_id=user_id,
        type='skill_endorsed',
        content=f'{User.query.get(current_user_id).name} endorsed your {skill} skill'
    )
    db.session.add(notif)
    db.session.commit()
...
@app.route('/api/users/<int:user_id>/endorsements', methods=['GET'])
def get_user_endorsements(user_id):
    """Get all endorsements for a user"""
    endorsements = SkillEndorsement.query.filter_by(endorsee_id=user_id).all()
    
    # Group by skill
    skills_dict = {}
    for e in endorsements:
        if e.skill not in skills_dict:
            skills_dict[e.skill] = []
        skills_dict[e.skill].append({
            'endorser_id': e.endorser.id,
            'endorser_name': e.endorser.name,
            'created_at': e.created_at.isoformat()
        })
    ...
    return jsonify({
        ...
        'skills': [{
            ...
            'count': len(endorsers),
            'endorsers': endorsers
        } for skill, endorsers in skills_dict.items()]
    })
...
def advanced_project_search():
    ...
    if search_term:
        query = query.filter(
            (Project.title.contains(search_term)) | 
    ...
    if skills:
        ...
        for skill in skill_list:
            query = query.filter(Project.skills_required.contains(skill.strip()))
    
    # Duration filter
    duration = request.args.get('duration')
    if duration:
        query = query.filter(Project.duration == duration)
    ...
    if sort_by == 'budget':
        query = query.order_by(Project.budget.desc() if sort_order == 'desc' else Project.budget.asc())
    else:
...
def get_analytics_overview():
    ...
    if user.role == 'client':
        ...
        total_spent = sum(c.total_paid for c in contracts)
        active_contracts = len([c for c in contracts if c.status == 'active'])
        
        # Monthly spending
        from datetime import timedelta
        thirty_days_ago = datetime.utcnow() - timedelta(days=30)
        recent_payments = Payment.query.join(Contract).join(Project).filter(
            Project.client_id == current_user_id,
            Payment.paid_at >= thirty_days_ago
        ).all()
        monthly_spending = sum(p.amount for p in recent_payments)
        ...
        return jsonify({
            ...
            'total_proposals_received': sum(len(p.proposals) for p in projects)
        })
        ...
        total_earned = sum(c.total_paid for c in contracts)
        
        # Time tracking stats
        time_entries = TimeEntry.query.filter_by(freelancer_id=current_user_id).all()
        total_hours = sum(e.hours for e in time_entries)
        
        # Monthly earnings
        from datetime import timedelta
        thirty_days_ago = datetime.utcnow() - timedelta(days=30)
        recent_payments = Payment.query.join(Contract).filter(
            Contract.freelancer_id == current_user_id,
            Payment.paid_at >= thirty_days_ago
        ).all()
        monthly_earnings = sum(p.amount for p in recent_payments)
